...
[**Short running title**]{.underline}: FAIRification of pharmacoepidemiological research
...
`Git` is a powerful tool that perfectly integrates into workflows of quantitative disciplines which have to handle analytic code. It further provides features that make it convenient for project-oriented setups and to collaborate at scale. This tutorial aimed to give an introduction to essential concepts that make it useful for pharmacoepidemiology research, but is by no means comprehensive as `Git` provides a vast amount of additional features which were not addressed in this manuscript. To get familiar with more advanced `Git` usage, we compiled a non-exhaustive list of further readings in the **Supplementary Material**.

`Git` was primarily designed to work with source code files and works best for plain, text-based documents such programming code, rich text format (RTF), Markdown or LaTex files. There are limitations when it comes to tracking binary files like Word (.docx) and Excel (.xlsx) as these need to be interpreted by a program or a hardware processor[@blischak2016]. While it can be still useful to track versions of these file types, it is important to recognize that modifications in such documents can't be easily inspected using Rstudio or GitHub.
...
Despite its potential initial complexities to get acquanited with the `Git` workflow, distributed VCS and code sharing provide unique advantages:
...
include_graphics(here("figures", "Figure_6_commit_Github_gui.png"))
```
