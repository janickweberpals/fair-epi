...
## Example for an end-to-end workflow using Git

This section aims to summarize a minimally commented end-to-end workflow of incorporating and tracking changes for a an analysis step of a RWE study. To that end, we use the same example as in the main manuscript.
...
# we use  data from the smdi R package
data <- smdi::smdi_data_complete

# step 1: compute propensity of exposure assignment

# 1.1 select variables to include in propensity 
ps_covars <- data %>% 
  dplyr::select(-exposure) %>% 
  names()

# 1.2: define formula for the GLM
exposure_form <- as.formula(paste("exposure ~ ", paste(ps_covars, collapse = " + ")))
...
# step 3: plot density/overlap before matching
